...
If the observations are assumed to be from a simple random sample, so we can potentially generalize to the population of Risso's dolphins from the Taiji area in Japan at the time of this study. We do not have information on dependencies that suggest a violation (such as sibling dolphins or different groups of sampling locations or dates that might relate to non-independent responses),
...
$$
\begin{aligned}
\text{point estimate} \pm t^{\star}_{df}
  \times SE(\text{point estimate})
&\;\to\;
\bar{y} \pm t^{\star}_{df}
  \times \frac{s}{\sqrt{n}}
\end{aligned}
$$
